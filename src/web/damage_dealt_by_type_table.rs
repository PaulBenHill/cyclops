...
pub fn process(context: &mut Context, query: &TableQuery) {
    ...
    match db_actions::get_damage_dealt_by_type_query(&query) {
        ...
        None => println!("Damage dealt by type returned no data"),
    };
}
...
pub fn flatten(data: Vec<DamageDealtByType>) -> Vec<Vec<String>> {
...
pub fn sort(sort_field: String, sort_dir: SortDirection, data: &mut Vec<DamageDealtByType>) {
